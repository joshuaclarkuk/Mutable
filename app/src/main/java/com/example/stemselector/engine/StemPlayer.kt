...
class StemPlayer(coroutineScope: CoroutineScope) {
    ...
    fun play(stems: List<String>) {
        if (stemStreams.isEmpty()) {
        ...
        // Checks for existing AudioTrack instance and releases if so, then creates single audiotrack that all stems feed into
        audioTrack?.release()
            ...
            while (isPlaying) {
                ...
                for ((stemPath, stream) in stemStreams) {
                    ...
                    while (i < bytesRead - 1) {
                        ...
                        val sample = (stemBuffer[i].toInt() and 0xff) or (stemBuffer[i + 1].toInt() shl 8)
                        val currentMix = mixBuffer[i / 2].toInt()

                        // Add sample to mix, clamp to Short range to prevent distortion
                        mixBuffer[i / 2] = (currentMix + sample).coerceIn(Short.MIN_VALUE.toInt(), Short.MAX_VALUE.toInt()).toShort()
                        i += 2
                    }
                }
